 ...
 * @return {Object}
 */
async function getSeancesFromCats(db, params) {
  // async function getSeancesFromCats(db, idCats, idProg) {


  let seances = await execQuery.single(db, queries.seancesFromCats, params);
  ...
  seances = _(seances).mapValues(d => {
      return _({}).assign(
          _(d[0]).pick(["idCycle", "idCategorie", "idEvenement", "idSeance", "dateHeure", "idSalle", "typeSeance", "titreEvenement"]).value(), {
...
module.exports = async function (db, cycleConfig) {
  ...
  let seances = await getSeancesFromCats(db, [idCats, idProg]);
  // let seances = await getSeancesFromCats(db, idCats);

  let seancesMentions = await getSeancesMentionsFromCats(db, [idCats, idProg]);
  // let seancesMentions = await getSeancesMentionsFromCats(db, idCats);

  let seancesCopies = await getSeancesCopiesFromCats(db, [idCats, idProg]);
