 ...
 * @param {Array} avant Tableau d'objets { idFilm, titre, art }
 * @param {Array} apres Tableau d'objets { idFilm, titre, art }
 * @returns {String} Chaîne formatée
 */
function precedeSuivi(avant, apres) {
  avant = joinLast(", ", ", ", avant);
  apres = joinLast(", ", ", ", apres);

  if (avant !== "" && apres != "") {
    return `Film précédé de ${avant} et suivi de ${apres}.`;
  } else if (avant !== "") {
    return `Film précédé de ${avant}.`;
  } else if (apres !== "") {
    return `Film suivi de ${apres}.`;
  } else {
    return "";
  }
}
