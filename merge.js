...
(async function () {
  ...
  let seances = await helpers.readFileAsJson(`./data/cycles/${filename.seances}.json`);
  let films = await helpers.readFileAsJson(`./data/cycles/${filename.films}.json`);


  let filmsInSeances = _(seances).map(d => _(d.items).map(e => e.idFilm).value()).flatten().uniq().sort().value();
  let filmsInFilms = _(films).map(d => d.idFilm).sort().value();
  ...
  console.log(`Les données séances référencent ${filmsInSeances.length} films.`);
  ...
  if (diffFilmsSeances.length === 0 && diffSeancesFilms.length === 0) {
    ...
    if (diffFilmsSeances.length > 0) {
      ...
      console.log(`Aucune séance n'a été trouvée pour les films suivants : ${diffFilmsSeances.join(", ")}`);
    }
    if (diffSeancesFilms.length > 0) {
      // Il faudra pouvoir patcher films.json avec les données manquantes
      console.log(`Les films suivants ont des séances mais pas de données les concernant : ${diffFilmsSeances.join(", ")}`);
    }
  }
  ...
  seances = _(seances)
    .map(d => {
      let header = _(d).omit("items").value();
      return _(d.items).map(e => _({}).assign(e, header).value()).value();
    })
    .flatten()
    .value();
  ...
  await writeFile(
    `data/cycles/CYCLE${cycleConfig.idCycleProg}_ALL ${cycleConfig.titreCycle}.json`,
